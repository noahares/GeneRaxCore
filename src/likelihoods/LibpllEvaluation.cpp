...
void LibpllEvaluation::createAndSaveRandomTree(
    const std::string &alignmentFilename, const std::string &modelStrOrFile,
...
LibpllEvaluation::LibpllEvaluation(const std::string &newickStrOrFile,
    ...
    : _treeInfo(std::make_unique<PLLTreeInfo>(
          newickStrOrFile, isNewickAFile, alignmentFilename, modelStrOrFile)) {}

double LibpllEvaluation::raxmlSPRRounds(unsigned int minRadius,
                                        unsigned int maxRadius,
                                        unsigned int thorough,
                                        unsigned int toKeep, double cutoff) {
  ...
  return corax_algo_spr_round(
      getTreeInfo(), static_cast<int>(minRadius), static_cast<int>(maxRadius),
      static_cast<int>(toKeep),   // params.ntopol_keep
      static_cast<int>(thorough), // THOROUGH
      0,                          // int brlen_opt_method,
      RAXML_BRLEN_MIN, RAXML_BRLEN_MAX, RAXML_BRLEN_SMOOTHINGS, 0.1,
      (fabs(cutoff) < std::numeric_limits<double>::epsilon())
          ? 0
          : &cutoff_info, // cutoff_info_t * cutoff_info,
...
double LibpllEvaluation::optimizeBranches(double tolerance) {
  ...
  _treeInfo->getTreeInfo()->params_to_optimize[0] =
...
double LibpllEvaluation::optimizeAllParametersOnce(corax_treeinfo_t *treeinfo,
                                                   double tolerance) {
  ...
  if (params_to_optimize & CORAX_OPT_PARAM_SUBST_RATES) {
    new_loglh =
        -1 * corax_algo_opt_subst_rates_treeinfo(
                 treeinfo, 0, CORAX_OPT_MIN_SUBST_RATE,
                 CORAX_OPT_MAX_SUBST_RATE, RAXML_BFGS_FACTOR, tolerance);
  }

  /* optimize BASE FREQS */
  if (params_to_optimize & CORAX_OPT_PARAM_FREQUENCIES) {
    new_loglh = -1 * corax_algo_opt_frequencies_treeinfo(
                         treeinfo, 0, CORAX_OPT_MIN_FREQ, CORAX_OPT_MAX_FREQ,
                         RAXML_BFGS_FACTOR, tolerance);
  }

  /* optimize ALPHA */
  if (params_to_optimize & CORAX_OPT_PARAM_ALPHA) {
    new_loglh = -1 * corax_algo_opt_onedim_treeinfo(
                         treeinfo, CORAX_OPT_PARAM_ALPHA, CORAX_OPT_MIN_ALPHA,
                         CORAX_OPT_MAX_ALPHA, tolerance);
  }

  if (params_to_optimize & CORAX_OPT_PARAM_PINV) {
    new_loglh = -1 * corax_algo_opt_onedim_treeinfo(
                         treeinfo, CORAX_OPT_PARAM_PINV, CORAX_OPT_MIN_PINV,
                         CORAX_OPT_MAX_PINV, tolerance);
  }

  /* optimize FREE RATES and WEIGHTS */
  if (params_to_optimize & CORAX_OPT_PARAM_FREE_RATES) {
    new_loglh = -1 * corax_algo_opt_rates_weights_treeinfo(
                         treeinfo, RAXML_FREERATE_MIN, RAXML_FREERATE_MAX,
                         RAXML_BRLEN_MIN, RAXML_BRLEN_MAX, RAXML_BFGS_FACTOR,
                         tolerance);
  ...
  if (params_to_optimize & CORAX_OPT_PARAM_BRANCHES_ITERATIVE) {
    ...
    new_loglh =
        -1 * corax_opt_optimize_branch_lengths_local_multi(
                 treeinfo->partitions, treeinfo->partition_count,
                 treeinfo->root, treeinfo->param_indices,
                 treeinfo->deriv_precomp, treeinfo->branch_lengths,
                 treeinfo->brlen_scalers, RAXML_BRLEN_MIN, RAXML_BRLEN_MAX,
                 tolerance,
                 static_cast<int>(brlen_smooth_factor *
                                  static_cast<double>(RAXML_BRLEN_SMOOTHINGS)),
                 -1, /* radius */
                 1,  /* keep_update */
                 CORAX_OPT_BLO_NEWTON_SAFE, treeinfo->brlen_linkage,
                 treeinfo->parallel_context, treeinfo->parallel_reduce_cb);
  }
  ...
  if (treeinfo->brlen_linkage == CORAX_BRLEN_SCALED &&
      treeinfo->partition_count > 1) {
    new_loglh =
        -1 * corax_algo_opt_onedim_treeinfo(
                 treeinfo, CORAX_OPT_PARAM_BRANCH_LEN_SCALER,
                 RAXML_BRLEN_SCALER_MIN, RAXML_BRLEN_SCALER_MAX, tolerance);
...
void LibpllEvaluation::invalidateCLV(unsigned int nodeIndex) {
  ...
  corax_treeinfo_invalidate_pmatrix(_treeInfo->getTreeInfo(),
                                    getNode(nodeIndex));
}
