...
public:
  ...
  corax_unode_t *&getRoot() { return _bestRoots[_bestRoots.size() / 2]; }
   ...
   */
  void fillUpTraversal(corax_unode_t *node, TaxaSet &set);
  ...
  corax_unode_t *getThirdNode(corax_unode_t *n1, corax_unode_t *n2) {
...
private:
  ...
  struct CLV {
    ...
    Clade clade;
    corax_unode_t *up;
    CLV() : score(0), isDup(false), isRoot(false), up(nullptr) {}
  };
  ...
  struct TaggerUNodePrinter {
    ...
    std::vector<CLV> &clvs;
    void operator()(corax_unode_t *node, std::stringstream &ss) const;
  };
};
