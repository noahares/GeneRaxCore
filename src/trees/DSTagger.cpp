...
DSTagger::DSTagger(PLLUnrootedTree &tree)
    : _tree(tree), _isRootDup(false), _clvs(_tree.getDirectedNodeNumber() + 3) {
...
void DSTagger::fillUpTraversal(corax_unode_t *node, TaxaSet &set) {
...
void DSTagger::_tagNode(corax_unode_t *node, CLV &clv) {
  ...
  clv.isDup = clv.clade.size() != (leftClade.size() + rightClade.size());
  if (clv.isDup) {
...
void DSTagger::print() {
  ...
  Logger::info << _tree.getNewickString(printer, root, true) << std::endl;
}
...
static void _fillWithChildren(corax_unode_t *node, TaxaSet &set) {
  if (!node->next) {
...
void DSTagger::fillWithChildren(corax_unode_t *node, TaxaSet &set) {
  orientUp(node);
...
static void
_fillWithInternalDescendants(corax_unode_t *node,
...
void DSTagger::fillWithInternalDescendants(
    corax_unode_t *node, std::vector<corax_unode_t *> &descendants) {
  if (node->next) {
    _fillWithInternalDescendants(node->next->back, descendants);
    _fillWithInternalDescendants(node->next->next->back, descendants);
  }
