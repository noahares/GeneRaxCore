 ...
 *
 * This is useful for implementing an iterator whose arrow operator would return
 * an r-value. Inspired by https://stackoverflow.com/a/26496041/4184258
 */
template <typename T> class ArrowOperatorProxy {
public:
  explicit ArrowOperatorProxy(T const &v) : t(v) {}
...
inline constexpr std::size_t hash_combine_64(std::size_t seed, T const &value) {
  return seed ^ (std::hash<T>()(value) + 0x9e3779b97f4a7c16 + (seed << 12) +
                 (seed >> 4));
}

/**
 * @brief Combine a seed value (e.g., another hash) with the hash of a given
 * type.
 */
template <typename T>
inline constexpr std::size_t hash_combine(std::size_t seed, T const &value) {
  ...
  // The inner functions use the golden ratio phi = (1 + sqrt(5))/2 as an
  // irrational number with random independent bits, by using its inverse and
  // the max size: 2^64 / phi = 0x9e3779b97f4a7c16 for the 64bit version for
  // example. Furthermore, shifting is added in order to spread bits around for
  // greater diversity. This whole approach follows the Boost hash combine
  // functions.

  static_assert(
      (sizeof(std::size_t) == 4) || (sizeof(std::size_t) == 8),
      "Need sizeof( std::size_t ) to be 4 or 8 (32bit or 64bit integer)");
  return (sizeof(std::size_t) == 4) ? hash_combine_32(seed, value)
