...
bool SpeciesSPRSearch::SPRRound(SpeciesTree &speciesTree,
    ...
    unsigned int radius)
{
  Logger::timed << "[Species search] Start new SPR round, radius=" << radius << std::endl;
    ...
    unsigned int radius)
{
  ...
  Logger::timed << "[Species search] After local SPR search: LL=" << searchState.bestLL << std::endl;
  return better;
}
