 ...
 */
struct UNNIMove {
  UNNIMove(corax_unode_t *e, bool l) : edge(e), left(l) {}
  void apply();
  corax_unode_t *getA() const { return edge->next->back; }
  corax_unode_t *getB() const { return edge->next->next->back; }
  corax_unode_t *getC() const {
    return left ? edge->back->next->back : edge->back->next->next->back;
  }
  corax_unode_t *getD() const {
    return !left ? edge->back->next->back : edge->back->next->next->back;
  }
...
class UNNISearch {
public:
  UNNISearch(PLLUnrootedTree &tree, USearchEvaluator &evaluator)
      : _tree(tree), _evaluator(evaluator) {}
  double runRound();
  double search();
