...
std::shared_ptr<SPRMove>
SPRMove::createSPRMove(unsigned int pruneIndex, unsigned int regraftIndex,
                       const std::vector<unsigned int> &path) {
  return std::make_shared<SPRMove>(pruneIndex, regraftIndex, path);
}

static void
optimizeBranchesSlow(JointTree &tree,
  ...
  for (unsigned int j = 0; j < 2; ++j) {
    for (unsigned int i = 0; i < nodesToOptimize.size(); ++i) {
      ...
      double newLoglk = corax_opt_optimize_branch_lengths_local(
          treeinfo->partitions[0], treeinfo->root, &params_indices[0],
          RAXML_BRLEN_MIN, RAXML_BRLEN_MAX, RAXML_BRLEN_TOLERANCE,
          RAXML_BRLEN_SMOOTHINGS, 0, true);
...
SPRMove::SPRMove(unsigned int pruneIndex, unsigned int regraftIndex,
                 const std::vector<unsigned int> &path)
    : _pruneIndex(pruneIndex), _regraftIndex(regraftIndex), _path(path) {}
...
std::shared_ptr<SPRRollback> SPRMove::applyMove(JointTree &tree) {
  ...
  return std::make_shared<SPRRollback>(tree, corax_rollback, savedBranches,
                                       root);
}
...
void SPRMove::reOptimizeMove(JointTree &tree) {
  ...
  for (unsigned int i = 0; i < _optimizedBranches.size(); ++i) {
    ...
    if (saved.getNode() != savedBack.getNode()->back) {
      Logger::info
          << "The branch pattern changed, skipping this branch reoptimization"
          << std::endl;
      continue;
    }
    saved.restore();
