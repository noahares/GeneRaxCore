...
struct SPRMove {
  ...
  double score;
  SPRMove(corax_unode_t *pruneNode, corax_unode_t *regraftNode, double score)
      : pruneNode(pruneNode), regraftNode(regraftNode), score(score) {}
  ...
  */
  ...
  bool operator==(const SPRMove &other) const { return other.score == score; }
};
...
public:
  Astrid(const PLLUnrootedTree &speciesTree, const Families &families,
         double minbl);
  virtual ~Astrid() {}
