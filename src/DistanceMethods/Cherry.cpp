...
class CherryTree {
public:
  CherryTree(const std::string &treeString, const GeneSpeciesMapping &mapping,
  ...
  void mergeNodesWithSpeciesId(unsigned int speciesId1, unsigned int speciesId2,
                               double &sumBL1, double &denBL1, double &sumBL2,
                               double &denBL2);
...
private:
  ...
  void updateInternodeDistance(int geneId, int parentGeneId,
                               int sourceSpeciesId, double currentDistance,
                               MatrixDouble &speciesDistance,
                               MatrixDouble &denominator, MatrixDouble &count);
  int getThirdIndex(CherryNode &node, int id1, int id2);
  void connect(CherryNode &node, int previousId, int newId);
...
void CherryTree::updateInternodeDistance(int geneId, int parentGeneId,
                                         int sourceSpeciesId,
  ...
  updateInternodeDistance(children.first, geneId, sourceSpeciesId,
                          currentDistance + 1.0, speciesDistances, denominator,
                          count);
  updateInternodeDistance(children.second, geneId, sourceSpeciesId,
                          currentDistance + 1.0, speciesDistances, denominator,
                          count);
}
...
void CherryTree::updateMiniNJMatrix(MatrixDouble &distanceMatrixToUpdate,
  ...
  for (auto &p : _speciesIdToGeneIds) {
    ...
    for (auto geneId : geneIdSet) {
      auto &node = _nodes[geneId];
      updateInternodeDistance(node.sons[0], geneId, speciesId, node.height,
                              speciesDistance, denominator, count);
    }
  }
  ...
  */
  ...
  for (auto &p : _speciesIdToGeneIds) {
    ...
    for (auto &p2 : _speciesIdToGeneIds) {
      ...
      (*denominatorMatrix)[speciesId][spid2] += 2.0 / (1.0 / big + 1.0 / small);
    ...
    for (unsigned int i = 0; i < speciesNumber; ++i) {
      for (unsigned int j = 0; j < speciesNumber; ++j) {
        if ((*denominatorMatrix)[i][j] != 0.0) {
          neighborMatrixToUpdate[i][j] += (*neighborMatrix)[i][j] /
  (*denominatorMatrix)[i][j]; neighborMatrixToUpdate[i][j] += 1.0;
        }
      }
    }
  }
...
int CherryTree::coveredSpeciesNumber() { return _speciesIdToGeneIds.size(); }
...
int CherryTree::mergeNeighbors(int geneId1, int geneId2) {
  ...
  for (int i = 0; i < 3; ++i) {
    if (parent.sons[i] != geneId1 && parent.sons[i] != geneId2) {
  ...
  _leavesNumber++;
  parent.speciesClade.insert(node1.speciesClade.begin(),
                             node1.speciesClade.end());
  parent.speciesClade.insert(node2.speciesClade.begin(),
                             node2.speciesClade.end());
  erase(node1.geneId);
  erase(node2.geneId);
...
int CherryTree::getThirdIndex(CherryNode &node, int id1, int id2) {
  for (int i = 0; i < 3; ++i) {
...
void CherryTree::connect(CherryNode &node, int previousId, int newId) {
  for (int i = 0; i < 3; ++i) {
...
void CherryTree::mergeNodesWithSpeciesId(unsigned int speciesId1,
                                         ...
                                         double &sumBL1, double &denBL1,
                                         double &sumBL2, double &denBL2) {
...
static void
filterGeneTrees(std::vector<std::shared_ptr<CherryTree>> &geneTrees) {
  auto geneTreesCopy = geneTrees;
  geneTrees.clear();
  for (auto geneTree : geneTreesCopy) {
    if (geneTree->getLeafNumber() >= 4 &&
        geneTree->coveredSpeciesNumber() > 2) {
...
Cherry::geneTreeCherry(const Families &families) {
  ...
  for (auto &family : families) {
    ...
    while (std::getline(reader, line)) {
      geneTrees.push_back(
          std::make_shared<CherryTree>(line, mapping, speciesStrToId));
    }
  }
  ...
  for (unsigned int i = 0; i < speciesNumber - 2; ++i) {
    ...
    if (CHERRY_DBG) {
      for (int i = 0; i < std::min(int(geneTrees.size()), TREES_TO_DISPLAY);
           ++i) {
        Logger::info << "Tree " << geneTrees[i]->_hackIndex << " "
                     << geneTrees[i]->toString() << std::endl;
      }
    }
    ...
    */
    ...
    std::pair<int, int> bestPairSpecies = MININJ
                                              ? getMinInMatrix(neighborMatrix)
                                              : getMaxInMatrix(neighborMatrix);
    ...
    for (auto geneTree : geneTrees) {
      geneTree->mergeNodesWithSpeciesId(bestPairSpecies.first,
                                        bestPairSpecies.second, sumBL1, denBL1,
                                        sumBL2, denBL2);
    ...
    speciesIdToStr[bestPairSpecies.first] =
        std::string("(") + speciesStr1 + ":" + std::to_string(bl1) + "," +
  ...
  double denBL2 = 0.0;
  for (auto geneTree : geneTrees) {
    geneTree->mergeNodesWithSpeciesId(lastSpeciesId[0], lastSpeciesId[1],
                                      sumBL1, denBL1, sumBL2, denBL2);
  }
  double bl1 = sumBL1 / denBL1;
  double bl2 = sumBL2 / denBL2;
  if (bl1 != bl1) {
