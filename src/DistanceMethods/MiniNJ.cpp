...
void fillDistancesRec(corax_unode_t *currentNode, double currentDistance,
                      std::vector<double> &distances,
                      double contractBranchUnder) {
  // do not account for internal nodes which have too small
  // branch lengths
  if (currentNode->length == 0.0 ||
      currentNode->length >= contractBranchUnder) {
    currentDistance += 1.0;
  }
  if (!currentNode->next) {
    // leaf
...
void MiniNJ::geneDistancesFromGeneTree(PLLUnrootedTree &geneTree,
  ...
  if (speciesNumber < 3) {
    Logger::error
        << "Found less than 3 species, please check that the genes cover at "
           "least 3 species or that the gene-species mappings are correct"
        << std::endl;
      ...
      if (!minMode) {
        ...
        if (distancesDenominatorToAdd[spid1][spid2]) {
          distancesToAdd[spid1][spid2] =
              std::min(distancesToAdd[spid1][spid2], leafDistances[gid1][gid2]);
        } else {
          distancesToAdd[spid1][spid2] = leafDistances[gid1][gid2];
...
std::unique_ptr<PLLRootedTree> MiniNJ::geneTreeNJ(const Families &families,
                                                  bool minMode, bool ustar,
                                                  bool reweight,
                                                  double contractBranchUnder) {
  DistanceMatrix distanceMatrix;
  std::vector<std::string> speciesIdToSpeciesString;
  StringToUint speciesStringToSpeciesId;
  computeDistanceMatrix(families, minMode, reweight, ustar, contractBranchUnder,
                        distanceMatrix, speciesIdToSpeciesString,
                        speciesStringToSpeciesId);
  auto res = NeighborJoining::applyNJ(distanceMatrix, speciesIdToSpeciesString,
                                      speciesStringToSpeciesId);
  return res;
}

void MiniNJ::computeDistanceMatrix(
    const Families &families, bool minMode, bool reweight, bool ustar,
    double contractBranchUnder, DistanceMatrix &distanceMatrix,
    std::vector<std::string> &speciesIdToSpeciesString,
    StringToUint &speciesStringToSpeciesId) {
  ...
  for (auto &family : perCoreFamilies) {
    ...
    for (const auto &str : geneTreeStrings) {
      ...
      geneDistancesFromGeneTree(geneTree, mappings, speciesStringToSpeciesId,
                                distanceMatrix, distanceDenominator, minMode,
                                reweight, ustar, contractBranchUnder);
    }
  }
