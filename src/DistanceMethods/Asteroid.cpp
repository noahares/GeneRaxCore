...
static BoolMatrix getBoolMatrix(unsigned int N, unsigned int M,
                                bool value = false) {
...
static void
getPrunedSpeciesMatrix(const PLLUnrootedTree &speciesTree,
                       const StringToUint &speciesStringToSpeciesId,
  ...
  for (auto node : speciesTree.getPostOrderNodes()) {
    ...
    if (node->next) {
      ...
      belongsToPruned[index] =
          hasChildren[leftIndex] && hasChildren[rightIndex];
    } else {
      bool isCovered =
          coverage.find(std::string(node->label)) != coverage.end();
      hasChildren[index] = isCovered;
      belongsToPruned[index] = isCovered;
    }
  }
  fillSpeciesDistances(speciesTree, speciesStringToSpeciesId, distanceMatrix,
                       &belongsToPruned);
}

Asteroid::Asteroid(const PLLUnrootedTree &speciesTree, const Families &families,
                   double minbl) {
  ...
  for (unsigned int i = 0; i < _K; ++i) {
    ...
    _perFamilyCoverage[i] =
        std::vector<bool>(speciesTree.getLeafNumber(), false);
    ...
    while (std::getline(reader, geneTreeStr)) {
      ...
      MiniNJ::geneDistancesFromGeneTree(
          geneTree, mappings, _speciesStringToSpeciesId,
          _geneDistanceMatrices[i], _geneDistanceDenominators[i], minMode,
          reweight, ustar, minbl);
    }
  }
  Logger::info << "Families: " << _K << std::endl;
  Logger::info << "Patterns: " << coverageSet.size() << std::endl;
  _prunedSpeciesMatrices =
      std::vector<DistanceMatrix>(_K, getNullMatrix(speciesNumber));
  auto nodesNumber = speciesTree.getDirectedNodeNumber();
  Logger::timed << "Asteroid::Asteroid 3" << std::endl;
  auto zero =
      getMatrix(nodesNumber, _K, std::numeric_limits<double>::infinity());
...
double Asteroid::computeBME(const PLLUnrootedTree &speciesTree) {
  ...
  for (unsigned int k = 0; k < _K; ++k) {
    // O(n^2)
    getPrunedSpeciesMatrix(speciesTree, _speciesStringToSpeciesId,
                           _perFamilyCoverageStr[k], _prunedSpeciesMatrices[k]);
  }
  // O(kn^2)
  for (unsigned k = 0; k < _geneDistanceMatrices.size(); ++k) {
    for (unsigned int i = 0; i < N; ++i) {
      for (unsigned int j = 0; j < i; ++j) {
        ...
        res += _geneDistanceMatrices[k][i][j] *
               _pows[_prunedSpeciesMatrices[k][i][j]];
      }
    }
  }
...
void Asteroid::_computeSubBMEsPruneRec(corax_unode_t *n1, corax_unode_t *n2,
                                       BoolMatrix &treated) {
...
void Asteroid::_computeSubBMEsPrune(const PLLUnrootedTree &speciesTree) {
  ...
  _hasChildren = getBoolMatrix(nodesNumber, _K, false);
  ...
  */
  for (auto n1 : speciesTree.getPostOrderNodes()) {
    ...
    for (auto n2 : speciesTree.getPostOrderNodesFrom(n1->back)) {
      ...
      _computeSubBMEsPruneRec(n1, n2, treated);
    }
  }
}

void Asteroid::_getBestSPRRecMissing(
    ...
    std::vector<bool> Vsminus2HasChildren, // does Vsminus2 have children after
                                           // the previous moves
    std::vector<bool> Vsminus1HasChildren) // does Vsminus1 have children after
                                           // the previous moves
{
  ...
  double diff = 0.0;
  std::vector<double> delta_Vsminus1_Wp(
  ...
  for (unsigned int k = 0; k < K; ++k) {
    ...
    // The current NNI move affects the current family score if and only if each
    // of its 4 subtrees (after having recursively applied the previous NNI
    // moves!!) have children in the induced tree At this point, Wp has
    // children. Vsminus1HasChildren tells if the updated Vs has children
    // belongsToPruned == true iif both the node children (Ws and Vs->back) have
    // children
    bool applyDiff = Vsminus1HasChildren[k] &&
                     belongsToPruned[Vsminus1->back->node_index][k];
    ...
    // deltaCD and deltaBD are trivial because not affected by previous NNI
    // moves
    ...
    if (sprime[k] <= 1 && applyDiff) {
      ...
      deltaAC =
          _pows[sprime[k]] * (getCell(W0s[k]->node_index, Ws->node_index, k) -
      ...
      if (hasChildren[Wsminus1->node_index][k] && Vsminus2HasChildren[k]) {
      ...
      } else if (hasChildren[Wsminus1->node_index][k] &&
                 !Vsminus2HasChildren[k]) {
        ...
        deltaAB =
            std::numeric_limits<double>::infinity(); // won't be used anyway
      }
    }
    if (applyDiff) {
  ...
  if (Vs->back->next) {
    _getBestSPRRecMissing(
        s + 1, stopCriterion, sprime, W0s, Wp, Ws, Vs, delta_Vsminus1_Wp,
        Vs->back->next, Ls, bestRegraftNode, subBMEToUpdate, bestLs, bestS,
        belongsToPruned, hasChildren, Vsminus1HasChildren, VsHasChildren);
    _getBestSPRRecMissing(s + 1, stopCriterion, sprime, W0s, Wp, Ws, Vs,
                          delta_Vsminus1_Wp, Vs->back->next->next, Ls,
                          bestRegraftNode, subBMEToUpdate, bestLs, bestS,
                          belongsToPruned, hasChildren, Vsminus1HasChildren,
                          VsHasChildren);
  }
...
void Asteroid::getBestSPR(PLLUnrootedTree &speciesTree,
  ...
  for (auto pruneNode : speciesTree.getPostOrderNodes()) {
    ...
    if (getBestSPRFromPrune(maxRadiusWithoutImprovement, pruneNode,
                            bestRegraftNode, bestDiff, bestS)) {
...
bool Asteroid::getBestSPRFromPrune(unsigned int maxRadiusWithoutImprovement,
                                   ...
                                   double &bestDiff, unsigned int &bestS) {
  ...
  for (auto V0 : V0s) {
    ...
    for (auto V1 : V1s) {
      ...
      std::vector<bool> Vminus1HasChildren(
          V0HasChildren.size()); // won't be read at first iteration
      _getBestSPRRecMissing(s, stopCriterion, sprime, W0s, Wp, Wsminus1, V,
                            delta_V0_Wp, V1, 0.0, bestRegraftNode, _toUpdate,
                            bestDiff, bestS, _belongsToPruned, _hasChildren,
                            Vminus1HasChildren, V0HasChildren);
