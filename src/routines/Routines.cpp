...
void Routines::runRaxmlOptimization(Families &families,
                                    ...
                                    long &sumElapsedSec) {
  RaxmlMaster::runRaxmlOptimization(families, output, execPath, iteration,
                                    splitImplem, sumElapsedSec);
}

void Routines::optimizeGeneTrees(
    ...
    double recWeight, bool enableRec, bool enableLibpll, unsigned int sprRadius,
    unsigned int iteration, bool schedulerSplitImplem, long &elapsed,
    bool inPlace) {
  GeneRaxMaster::optimizeGeneTrees(
      families, recModelInfo, rates, output, resultName, execPath,
      speciesTreePath, reconciliationOpt, madRooting, supportThreshold,
      recWeight, enableRec, enableLibpll, sprRadius, iteration,
      schedulerSplitImplem, elapsed, inPlace);
}
...
void Routines::exportPerSpeciesRates(const std::string &speciesTreeFile,
  ...
  Logger::info << "Exporting per-species rates into " << outputFile
               << std::endl;
...
void Routines::optimizeRates(bool userDTLRates,
                             ...
                             const RecModelInfo &recModelInfo,
                             Families &families, bool perSpeciesRates,
                             Parameters &rates, long &sumElapsed) {
  if (userDTLRates) {
    return;
  }
  ...
  if (perSpeciesRates) {
    ...
    rates = DTLOptimizer::optimizeParametersPerSpecies(
        evaluations, speciesTree.getNodeNumber());
  } else {
...
static std::string getSpeciesEventCountFile(const std::string &outputDir,
...
static std::string getTransfersFile(const std::string &outputDir,
                                    const std::string &familyName,
                                    int sample = -1) {
  auto res = FileSystem::joinPaths(
      outputDir, FileSystem::joinPaths("reconciliations", familyName));
  if (sample >= 0) {
...
void Routines::inferAndGetReconciliationScenarios(
    PLLRootedTree &speciesTree, const PerCoreGeneTrees &geneTrees,
  ...
  for (unsigned int i = 0; i < geneTrees.getTrees().size(); ++i) {
    ...
    evaluations[i] = std::make_shared<ReconciliationEvaluation>(
        speciesTree, *tree.geneTree, tree.mapping, modelParameters.info,
  ...
  if (optimizeRates) {
    ...
    modelParameters = DTLOptimizer::optimizeModelParameters(
        evaluations, optimizeFromStartingParameters, modelParameters);
...
void Routines::inferReconciliation(const std::string &speciesTreeFile,
                                   Families &families,
                                   const ModelParameters &initialModelRates,
                                   ...
                                   bool optimizeRates) {
  PerCoreGeneTrees geneTrees(families);
  std::string reconciliationsDir =
      FileSystem::joinPaths(outputDir, "reconciliations");
  FileSystem::mkdir(reconciliationsDir, true);
  PLLRootedTree speciesTree(speciesTreeFile);
  if (bestReconciliation) {
    std::vector<std::shared_ptr<Scenario>> scenarios;
    inferAndGetReconciliationScenarios(
        speciesTree, geneTrees, initialModelRates, 0, optimizeRates, scenarios);
    ...
    Scenario::saveTransferPairCountGlobal(speciesTree, scenarios,
                                          transferPairGlobalFile);
  }
  if (reconciliationSamples) {
    std::vector<std::shared_ptr<Scenario>> scenarios;
    inferAndGetReconciliationScenarios(speciesTree, geneTrees,
                                       initialModelRates, reconciliationSamples,
                                       optimizeRates, scenarios);
    assert(scenarios.size() ==
           geneTrees.getTrees().size() * reconciliationSamples);
    for (unsigned int i = 0; i < geneTrees.getTrees().size(); ++i) {
      auto &tree = geneTrees.getTrees()[i];
      std::string nhxSamples =
          FileSystem::joinPaths(reconciliationsDir, tree.name + "_samples.nhx");
  ...
  ParallelContext::barrier();
  {
    ...
    getPerSpeciesEvents(speciesTree, geneTrees, initialModelRates,
                        reconciliationSamples, events, forceTransfers);
    PLLRootedTree speciesTree(speciesTreeFile);
    ParallelOfstream os(
        FileSystem::joinPaths(outputDir, "per_species_event_counts.txt"));
...
void Routines::computeSuperMatrixFromOrthoGroups(
  ...
  PLLRootedTree speciesTree(speciesTreeFile);
  const std::unordered_set<std::string> speciesSet(speciesTree.getLabels(true));
  std::string reconciliationsDir =
      FileSystem::joinPaths(outputDir, "reconciliations");
  ...
  for (auto &family : families) {
    std::string orthoGroupFile =
        FileSystem::joinPaths(reconciliationsDir, family.name);
    if (largestOnly) {
    ...
    for (auto &orthoGroup : orthoGroups) {
      ...
      LibpllParsers::parseMSA(family.alignmentFile, model->charmap(), sequences,
                              weights);
      ...
      for (auto &sequence : sequences) {
        ...
        if (orthoGroup->find(geneLabel) != orthoGroup->end()) {
          ...
          superMatrix[mapping.getSpecies(geneLabel)] +=
      ...
      partitionOs << " = " << offset - currentSize + 1 << "-" << offset
                  << std::endl;
...
bool Routines::createRandomTrees(const std::string &geneRaxOutputDir,
                                 Families &families) {
  ...
  for (auto &family : families) {
    if (family.startingGeneTree == "__random__") {
      ...
      family.startingGeneTree =
          FileSystem::joinPaths(geneRaxOutputDir, "startingTrees");
      family.startingGeneTree = FileSystem::joinPaths(family.startingGeneTree,
                                                      family.name + ".newick");
      if (ParallelContext::getRank() == 0) {
        LibpllEvaluation::createAndSaveRandomTree(
            family.alignmentFile, family.libpllModel, family.startingGeneTree);
      }
    }
  }
  Random::setSeed(consistentSeed);
...
void Routines::gatherLikelihoods(Families &families, double &totalLibpllLL,
                                 double &totalRecLL) {
  ParallelContext::barrier();
  totalRecLL = 0.0;
  totalLibpllLL = 0.0;
  unsigned int familiesNumber = static_cast<unsigned int>(families.size());
  for (auto i = ParallelContext::getBegin(familiesNumber);
       i < ParallelContext::getEnd(familiesNumber); ++i) {
...
void Routines::getLabelsFromTransferKey(const std::string &key,
                                        std::string &label1,
                                        std::string &label2) {
...
void Routines::getPerSpeciesEvents(PLLRootedTree &speciesTree,
  ...
  if (Enums::accountsForTransfers(modelParameters.info.model) ||
    ...
    transfersModelParameter =
        ModelParameters(transfersParameters, 1, recModelInfo);
  }
  transfersModelParameter.info.rootedGeneTree = false;
  inferAndGetReconciliationScenarios(
      speciesTree, geneTrees, transfersModelParameter, reconciliationSamples,
      optimizeRates, scenarios);
...
void Routines::getTransfersFrequencies(
    PLLRootedTree &speciesTree, PerCoreGeneTrees &geneTrees,
    const ModelParameters &modelParameters, unsigned int reconciliationSamples,
...
void Routines::buildEvaluations(PerCoreGeneTrees &geneTrees,
  ...
  for (unsigned int i = 0; i < trees.size(); ++i) {
    ...
    evaluations[i] = std::make_shared<ReconciliationEvaluation>(
        speciesTree, *tree.geneTree, tree.mapping, recModelInfo,
        forcedRootedGeneTree);
  }
}
