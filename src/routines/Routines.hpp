...
public:
  ...
  computeInitialSpeciesTree(Families &family, const std::string globalOutputDir,
  ...
  static void getTransfersFrequencies(
      PLLRootedTree &speciesTree, PerCoreGeneTrees &geneTrees,
      const ModelParameters &modelRates, unsigned int reconciliationSamples,
  ...
  static void
  getParametersFromTransferFrequencies(const std::string &speciesTreeFile,
   ...
   */
  static void
  inferReconciliation(const std::string &speciesTreeFile, Families &families,
                      const ModelParameters &initialModelRates,
                      const std::string &outputDir, bool bestReconciliation,
  ...
  static void inferAndGetReconciliationScenarios(
      ...
      bool optimizeRates, std::vector<std::shared_ptr<Scenario>> &scenarios);
  ...
  static void computeSuperMatrixFromOrthoGroups(
      ...
      bool largestOnly, bool masterOnly = true);
  ...
  /**
   * Read the family stats files and sum the sequence and reconciliation
   * likelihoods
   */
  static void gatherLikelihoods(Families &families, double &totalLibpllLL,
                                double &totalRecLL);
