/**
 * This is a simpler reimplementation of the raxml-ng search algorithm with
 * autodetect, fast and slow SPR roudns
 */
...
static void optimizeParameters(LibpllEvaluation &evaluation, double radius) {
  ...
  Logger::timed << "[" << initialLL << "]" << " Optimize parametres (" << radius
                << ")" << std::endl;
...
static bool optimizeTopology(LibpllEvaluation &evaluation,
                             unsigned int radiusMin, unsigned int radiusMax,
                             unsigned int thorough, unsigned int toKeep,
                             double cutoff) {

  double initialLL = evaluation.computeLikelihood(false);
  Logger::timed << "[" << initialLL << "] " << (thorough ? "SLOW" : "FAST")
                << " SPR Round (radius=" << radiusMax << ")" << std::endl;
  double ll =
      evaluation.raxmlSPRRounds(radiusMin, radiusMax, thorough, toKeep, cutoff);
...
int RaxmlSlave::runRaxmlOptimization(int argc, char **argv, void *comm) {
  ...
  LibpllEvaluation evaluation(startingGeneTreeFile, true, alignmentFile,
                              libpllModel);
  ...
  unsigned int radiusLimit = std::min(
      22u, static_cast<unsigned int>(evaluation.getTreeInfo()->tip_count - 3));
  ...
  cutoff = 1.0;
  while (
      optimizeTopology(evaluation, radiusMin, radiusMax, 0, toKeep, cutoff)) {
  }
  thorough = 1;
