...
struct PLLSequence {
  PLLSequence(char *label_, char *seq_, unsigned int len_)
      : label(label_), seq(seq_), len(len_) {}
  char *label;
  char *seq;
  unsigned int len;
  ~PLLSequence() {
...
public:
  ...
  static double getMSAEntropy(const std::string &alignmentFilename,
                              const std::string &modelStrOrFilename);
  static void fillLeavesFromUtree(corax_utree_t *utree,
                                  std::unordered_set<std::string> &leaves);
  static void fillLeavesFromRtree(corax_rtree_t *rtree,
                                  std::unordered_set<std::string> &leaves);
...
private:
   ...
   */
  static void parseFasta(const char *fasta_file, const corax_state_t *stateMap,
                         PLLSequencePtrs &sequences, unsigned int *&weights);
