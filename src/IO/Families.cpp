...
static std::string getErrorMessage(FamilyErrorCode error) {
  ...
  case ERROR_OK:
    assert(0);
    return "";
  case ERROR_READ_ALIGNMENT:
    return "Cannot read alignment file (file exists but is invalid)";
  case ERROR_INVALID_LABEL:
    return "Some labels in the MSAs contain invalid characters";
  case ERROR_READ_GENE_TREE:
    return "Cannot read starting gene tree file (file exists but is invalid)";
  case ERROR_GENE_TREE_POLYTOMY:
    return "Gene tree is not strictly binary";
  case ERROR_NOT_ENOUGH_GENES:
    return "The gene family should contain at least 3 sequences";
  case ERROR_GENE_TREE_SEQUENCES_MISMATCH:
    return "Mismatch between gene tree leaves and sequence labels";
  case ERROR_ALIGNEMENT_FILE_EXISTENCE:
    return "Input alignment file does not exist";
  case ERROR_GENE_TREE_FILE_EXISTENCE:
    return "Input starting gene tree file does not exist";
  case ERROR_MAPPING_FILE_EXISTENCE:
    return "A mapping file was given but does not exist";
  case ERROR_MAPPING_MISMATCH:
    return "Failed to map genes and species";
  case ERROR_NO_ALI_NO_TREE:
    return "This family does not have any starting gene tree nor alignment";
  }
  return "";
}

static FamilyErrorCode
filterFamily(const FamilyInfo &family,
             const std::unordered_set<std::string> &speciesTreeLabels,
             bool checkAlignments) {
  std::unordered_set<std::string> alignmentLabels;
  std::unordered_set<std::string> geneTreeLabels;
  bool geneTreeProvided =
      family.startingGeneTree != "__random__" && family.startingGeneTree.size();
  ...
  if (checkAlignments) {
    try {
      ...
      if (!LibpllParsers::fillLabelsFromAlignment(
              family.alignmentFile, family.libpllModel, alignmentLabels)) {
  ...
  if (geneTreeProvided) {
    ...
    try {
      geneTree = std::make_unique<PLLUnrootedTree>(family.startingGeneTree);
    } catch (const LibpllException &e) {
      std::cerr << e.what() << std::endl;
    } catch (...) {
    }
...
void Family::filterFamilies(Families &families,
  ...
  if (checkSpeciesTree) {
    if (!FileSystem::exists(speciesTreeFile)) {
      Logger::info << "[Error] Species tree file does not exist ("
                   << speciesTreeFile << ")" << std::endl;
    ...
    if (!speciesTree) {
      Logger::info << "[Error] Cannot parse species tree file ("
                   << speciesTreeFile << ")" << std::endl;
      ParallelContext::abort(10);
    }
    LibpllParsers::fillLeavesFromRtree(speciesTree, speciesTreeLabels);
  }
  std::vector<unsigned int> localErrors(
      (initialFamilySize - 1) / ParallelContext::getSize() + 1, 99999999);
  std::vector<unsigned int> errors;
  for (auto i = ParallelContext::getBegin(initialFamilySize);
       i < ParallelContext::getEnd(initialFamilySize); i++) {
    auto &family = copy[i];
    localErrors[i - ParallelContext::getBegin(initialFamilySize)] =
        filterFamily(family, speciesTreeLabels, checkAlignments);
  }
  ...
  for (unsigned int i = 0; i < initialFamilySize; ++i) {
    ...
    if (ERROR_OK == error) {
      ...
      Logger::info << "Error in family " << family.name << ": "
                   << getErrorMessage(static_cast<FamilyErrorCode>(error))
                   << std::endl;
      invalid++;
    }
  }
  if (invalid) {
    Logger::info
        ...
        << " invalid families (they will be discarded from the analysis)"
        << std::endl;
  }
  if (speciesTree) {
...
void Family::printStats(Families &families, const std::string &speciesTreeFile,
  ...
  Logger::info << "- Number of species: " << speciesTree.getLeafNumber()
               << std::endl;
  Logger::info << "- Total number of genes: " << totalGeneNumber << std::endl;
  Logger::info << "- Average number of genes per family: "
               << totalGeneNumber / families.size() << std::endl;
  Logger::info << "- Maximum number of genes per family: " << maxGeneNumber
               << std::endl;
  Logger::info << "- Species with the smallest family coverage: \""
               << minCoveredSpecies << "\" (covered by " << minSpeciesCoverage
               << "/" << families.size() << " families)" << std::endl;
  Logger::info << "- Average (over species) species family coverage: "
               << totalSpeciesCoverage / speciesLabels.size() << std::endl;
  Logger::info << std::endl;
}
