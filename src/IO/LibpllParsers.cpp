...
static char *
rtree_export_newick_recursive(const corax_rnode_t *root,
  ...
  if (!(root->left) || !(root->right)) {
    ...
    if (cb_serialize) {
      ...
      asprintf(&newick, "(%s,%s)%s", subtree1, subtree2, temp);
      free(temp);
    } else {
      asprintf(&newick, "(%s,%s)%s:%f", subtree1, subtree2,
...
static corax_rtree_t *readRooted(const std::string &newick, bool isFile) {
  ParsingError error;
  auto tree = custom_rtree_parse_newick(newick.c_str(), isFile, &error);
  if (error.type == PET_NOERROR) {
    ...
    if (isFile) {
      errorMessage =
          "Error while reading rooted tree from file " + newick + ".\n";
    } else {
      errorMessage =
          "Error while reading rooted tree from string " + newick + ".\n";
    }
...
corax_rtree_t *
LibpllParsers::readRootedFromFile(const std::string &newickFile) {
  return readRooted(newickFile, true);
}

corax_rtree_t *
LibpllParsers::readRootedFromStr(const std::string &newickString) {
...
void LibpllParsers::getUnodeNewickString(const corax_unode_t *rnode,
                                         std::string &newick) {
...
void rtreeHierarchicalStringAux(const corax_rnode_t *node,
...
void LibpllParsers::getRtreeHierarchicalString(const corax_rtree_t *rtree,
                                               std::string &newick) {
  std::ostringstream os;
  std::vector<bool> lefts;
...
LibpllParsers::parallelGetTreeSizes(const Families &families) {
  ...
  treeSizes.erase(remove(treeSizes.begin(), treeSizes.end(), 0),
                  treeSizes.end());
  assert(treeSizes.size() == families.size());
  return treeSizes;
}
void LibpllParsers::fillLeavesFromUtree(
    corax_utree_t *utree, std::unordered_set<std::string> &leaves) {
...
void LibpllParsers::fillLeavesFromRtree(
    corax_rtree_t *rtree, std::unordered_set<std::string> &leaves) {
...
void LibpllParsers::parseMSA(const std::string &alignmentFilename,
  ...
  if (!std::ifstream(alignmentFilename.c_str()).good()) {
    throw LibpllException("Alignment file " + alignmentFilename +
                          "does not exist");
  }
  try {
    parseFasta(alignmentFilename.c_str(), stateMap, sequences, weights);
  } catch (...) {
    parsePhylip(alignmentFilename.c_str(), stateMap, sequences, weights);
  }
}
...
void LibpllParsers::parseFasta(const char *fastaFile,
  ...
  int length;
  while (
      corax_fasta_getnext(reader, &head, &head_len, &seq, &seq_len, &seqno)) {
    sequences.push_back(PLLSequencePtr(
        new PLLSequence(head, seq, static_cast<unsigned int>(seq_len))));
    length = static_cast<int>(seq_len);
  }
  unsigned int count = static_cast<unsigned int>(sequences.size());
  char **buffer =
      static_cast<char **>(malloc(static_cast<size_t>(count) * sizeof(char *)));
  assert(buffer);
  for (unsigned int i = 0; i < count; ++i) {
    buffer[i] = sequences[i]->seq;
  }
  weights = corax_compress_site_patterns(buffer, stateMap,
                                         static_cast<int>(count), &length);
  if (!weights) {
    corax_fasta_close(reader);
    free(buffer);
    throw LibpllException(
        "Error while parsing fasta: cannot compress sites from ", fastaFile);
  }
  for (unsigned int i = 0; i < count; ++i) {
...
void LibpllParsers::parsePhylip(const char *phylipFile,
  ...
  assert(phylipFile);
  assert(stateMap);
  std::unique_ptr<corax_phylip_t, void (*)(corax_phylip_t *)> reader(
      corax_phylip_open(phylipFile, corax_map_phylip), corax_phylip_close);
  if (!reader) {
  ...
  try {
    ...
    std::unique_ptr<corax_phylip_t, void (*)(corax_phylip_t *)> reader2(
        corax_phylip_open(phylipFile, corax_map_phylip), corax_phylip_close);
  ...
  weights = corax_compress_site_patterns(msa->sequence, stateMap, msa->count,
                                         &msa->length);
  if (!weights)
    throw LibpllException("Error while parsing fasta: cannot compress sites");
  for (auto i = 0; i < msa->count; ++i) {
    PLLSequencePtr seq(new PLLSequence(msa->label[i], msa->sequence[i],
...
LibpllParsers::getModel(const std::string &modelStrOrFilename) {
...
bool LibpllParsers::fillLabelsFromAlignment(
    const std::string &alignmentFilename, const std::string &modelStrOrFilename,
  ...
  bool res = true;
  try {
    LibpllParsers::parseMSA(alignmentFilename, model->charmap(), sequences,
                            patternWeights);
  } catch (...) {
    res = false;
  }
...
unsigned int
LibpllParsers::getMSALength(const std::string &alignmentFilename,
